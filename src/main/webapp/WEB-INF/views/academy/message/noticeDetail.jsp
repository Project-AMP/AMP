...
<%
	NoticeVo notice_detail = (NoticeVo) request.getAttribute("notice_detail");
...
<script type="text/javascript">
	function notice_delete() {
		...
		if(confirm("정말 삭제하시겠습니까?")){
			...
			if(idx_ != null && academyidx_ != null)
			{
				location.href = "notice_delete.do?idx=" + <%=idx%> + "&academyidx=" + '<%=academyidx%>';
			}
			else
			{
				alert("취소하였습니다.");
...
<body>
	...
	<%if(Integer.parseInt(academyidx)<1) {%>
		<div id="topBarDiv"></div><script type="text/javascript">ajaxDo('topBar','#topBarDiv');</script>
	<%}else{ %>
		<div id="academyMain_topBarDiv"></div><script type="text/javascript">ajaxDo('academyMain_topBar?academyidx=<%=academyidx%>','#academyMain_topBarDiv');</script>
	<%} %>
	...
	<div class="btn_list_modify_delete" style=" margin-top: 15px;">
		<input type="button" value="목록" class="btn btn-dark" onclick="location.href='notice.do?academyidx=<%=academyidx%>'">
		<input type="button" value="수정" class="btn btn-dark" onclick="location.href='notice_update.do?idx=<%=idx%>&academyidx=<%= academyidx %>'">
		<input type="button" value="삭제" class="btn btn-dark" onclick="notice_delete();">
	</div>
	<br><br><br><br><br>
